...
class Cart_module_lib {
    ...
    public function validateCartMenuOption(&$menu_data, $menu_options) {
        if($this->CI->input->post('size_id')){
            $size_id = $this->CI->input->post('size_id');
            $price_string = $menu_data['menu_price'];

            

            if(is_string($price_string) && is_array(json_decode($price_string, true)) && (json_last_error() == JSON_ERROR_NONE)){
                $price = json_decode($price_string, true)[$size_id];
            }
        }
        ...
        if ($this->CI->input->post('menu_options') AND is_array($this->CI->input->post('menu_options'))) {
            $option_price = 0;
                        if($this->CI->input->post('size_id')){
                            $data_size['size_id'] = $size_id;
                            $this->CI->load->model('Size_options_model');
                            $res = $this->CI->Size_options_model->getSize($data_size);
                    ...
                    if ($menu_options[$menu_option_id]['required'] === '1'
                        ...
                        foreach ($menu_option['option_values'] as $key => $value) {
                            if (isset($option_values[$value], $option_values[$value]['menu_option_value_id'], $option_values[$value]['value'], $option_values[$value]['price'])) {
                                ...
                                if($this->CI->input->post('size_id')){
                                    if($this->isJSON($option_values[$value]['price'])){
                                        $option_values[$value]['price'] = json_decode($option_values[$value]['price'], true)[$size_id];
                                    }
                                }

                                $m_options = $this->CI->input->post('menu_options');
                                $side = null;

                                if(isset($m_options)){
                                    $side = $this->CI->input->post('side')[$menu_option_id][$menu_option['option_id']];
                                    $side = array_values($side)[$key];
                                }
                                if(isset($side)){
                                    $option_values[$value]['value'] .= ' / '.$side;
                                    if($side != 'whole'){
                                        $option_values[$value]['price'] /= 2;
                                    }
                                }
