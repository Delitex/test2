...
class Cart_module extends Main_Controller {
	...
	public function add() {																		// add() method to add item to cart
		...
		if ( ! $this->input->is_ajax_request()) {
			...
			if($this->input->post('size_id')){
				...
				$this->load->model('Size_options_model');
				$res = $this->Size_options_model->getSize($data_size);
	...
	public function options() {																	// _updateModule() method to update cart
		...
		if($this->isJSON($price_string)){
			$price_string = json_decode($price_string, true);
			$price = $price_string[$size_id];
		}
		else{
			$price = $price_string;
		}
		...
		$data['size_id'] 				= $this->input->get('size_id');
		$data['menu_name'] 				= $menu_data['menu_name'];
		$data['menu_price'] 			= $this->currency->format($price);
		...
		if ($menu_options = $this->Cart_model->getMenuOptions($this->input->get('menu_id'))) {
			foreach ($menu_options as $menu_id => $option) {
				...
				foreach ($option_values as $value) {
					if($this->isJSON($value['price'])){
						$price_string = json_decode($value['price'], true);
						$price = $price_string[$size_id];
						$value['price'] = $price;
						if($value['price'] == null){
							continue;
					...
					if($value['allow_half_size']){
						foreach ($cart_item['options'][$option['menu_option_id']] as $k1 => $v1) {
							if($v1['value_id']==$value['menu_option_value_id']){
								$t = $k1;
							}
						}
						if($t !== null){
							$side = $cart_item['options'][$option['menu_option_id']][$t]['value_side'];
						}
					}
					else{
						$side = 'whole';
