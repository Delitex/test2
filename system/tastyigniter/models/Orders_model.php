...
class Orders_model extends TI_Model {
    ...
    public function getCustomCount(){
        if (APPDIR === ADMINDIR) {
            $this->db->select('order_id, orders.status_id, status_name, status_color');
            $this->db->from('orders');
            $this->db->join('statuses', 'statuses.status_id = orders.status_id', 'left');
            $this->db->where('orders.status_id', 11);
    ...
    public function get_modal_status(){
            $this->db->from('modal');
    ...
    public function update_modal_status($dataArray = array(), $data){
        if (APPDIR === ADMINDIR) {

            if($data && $dataArray){

                $this->db->where($dataArray);
                $query=$this->db->update('modal', $data);
            }
        }
    }
    ...
    public function getCount($filter = array()) {
        if (APPDIR === ADMINDIR) {
			...
			if ( ! empty($filter['filter_employeeid'])) {
				$this->db->where('assignee_id', $filter['filter_employeeid']);
			}
