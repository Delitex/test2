	...
	public function addOptionValues($option_id = FALSE, $option_values = array()) {
		...
		if ($this->input->post()) {
			...
			foreach ($option_values as $key => $value) {
				if($this->input->post('allow_multiple_sizes'.$key)==='1'){

					$sizes_array = array();
					foreach ($sizes[$key] as $k => $v) {
						if($v && $size_v[$key][$k] && $v['price'])
						$sizes_array[$size_v[$key][$k]] = $v['price'];
					}
						$sizes_json = json_encode($sizes_array);

					if($this->isJSON($sizes_json) && $sizes_json){
						$option_values[$key]['price'] = $sizes_json;
					}

				}
				if($option_values[$key]['price'] == '[]'){
					$option_values[$key]['price'] = '0';
				}
			}
		...
		if ($option_id !== FALSE AND ! empty($option_values) AND is_array($option_values)) {
			...
			foreach ($option_values as $key => $value) {
				...
				if ($this->input->post('allow_half_size'.$key)) {
					$this->db->set('allow_half_size', $this->input->post('allow_half_size'.$key));
				}
