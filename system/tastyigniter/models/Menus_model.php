...
class Menus_model extends TI_Model {
	...
	public function getList($filter = array()) {
		...
		if ($this->db->limit($filter['limit'], $filter['page'])) {
			...
			foreach ($result as $row) {                                                            // loop through menus array
				...
				if(is_string($price) && is_array(json_decode($price, true)) && (json_last_error() == JSON_ERROR_NONE)){
					$str_price = '';
					$string_prices = json_decode($price, true);
					foreach ($string_prices as $key => $value) { 
						$string_prices[$key] = $this->currency->format($value);
					}
					$price = json_encode($string_prices);
				}
				else{
				...
				$results[$row['menu_category_id']][] = array(                                                            // create array of menu data to be sent to view
					...
					'menu_price'       => $price,        //add currency symbol and format price to two decimal places
	...
	public function saveMenu($menu_id, $save = array()) {
		...
		if ($this->input->post() && $this->input->post('allow_multiple_sizes')==='1') {
			$sizes = $this->input->post('option_size');
			$size_v = $this->input->post('sizes_v');

				
			$sizes_array = array();
			foreach ($sizes as $k => $v) {
				if($v && $size_v[$k] && $v['price'])
				$sizes_array[$size_v[$k]] = $v['price'];
			}
		...
		if (isset($save['menu_price'])) {
			if($this->isJSON($sizes_json) && $sizes_json){ 
				$this->db->set('menu_price', $sizes_json);
			}
			else{ 
