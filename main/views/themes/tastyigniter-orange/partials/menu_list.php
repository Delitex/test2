								...
								<div id="menu<?php echo $menu['menu_id']; ?>" class="menu-item">
									<div class="menu-item-wrapper row">
										...
										<div class="menu-right col-xs-4 wrap-none">
												<?php if(isJson($menu['menu_price'])){
														$string_sizes = json_decode($menu['menu_price'], true);
														$sorted_sizes = array();
									                    if(!empty($string_sizes)){
									                        foreach ($size_list as $k1 => $v1) {
														...
														foreach ($string_sizes as $key => $value) {
															foreach ($size_list as $sk => $sv) {
																if($sv->size_id==$key){
																	echo "<span class=' small'>".$sv->size_name." - </span>";
																}
															}
															echo " <span class='menu-price'>".$value."</span>";
												?>
													<span class="menu-button" style="float: right;">
														...
														<?php } else if (isset($menu_options[$menu['menu_id']])) { ?>
															<a class="btn btn-primary btn-cart add_cart" onClick="openMenuOptions('<?php echo $menu['menu_id']; ?>', '<?php echo $menu['minimum_qty']; ?>', '<?php echo $key; ?>');">
																<span class="fa fa-plus"></span>
															</a>
														<?php } else { ?>
															<a class="btn btn-primary btn-cart add_cart" onClick="addToCart('<?php echo $menu['menu_id']; ?>', '<?php echo $menu['minimum_qty']; ?>', '<?php echo $key; ?>');">
																<span class="fa fa-plus"></span>
															</a>
														<?php } ?>
													</span>

												<?php if(end($string_sizes)!=$value) { echo "<br><br>"; } }
													}
													else{
														echo "<span class='menu-price'>".$menu['menu_price']."</span>";
												?>
