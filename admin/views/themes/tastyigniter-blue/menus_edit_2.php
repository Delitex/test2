        ...
        <form role="form" id="edit-form" class="form-horizontal" enctype="multipart/form-data" accept-charset="utf-8" method="POST" action="<?php echo $_action; ?>">
            <div class="tab-content">
                <div id="general" class="tab-pane row wrap-all active">
                    ...
                    <?php $string_sizes_json = $menu_price;
                    
                    $string_sizes = json_decode($string_sizes_json, true);
                    $json_check = isJSON($string_sizes_json);
                    ?>
                    <div class="form-group" style="display: <?php if(!$json_check || empty($string_sizes)){ echo 'none';$allow_multiple_sizes = '0';} ?>;">
                        <label for="input-price" class="col-sm-3 control-label"><?php echo lang('label_price'); ?></label>
                        <div class="col-sm-5">
                            <div class="input-group">
                                <input type="text" name="menu_price" id="input-price" class="form-control" value="<?php if($json_check){ echo '0.0';} else{echo set_value('menu_price', $menu_price);} ?>" />
                    ...
                    <div class="form-group">
                        <label for="input-special-status" class="col-sm-3 control-label">Allow multiple sizes</label>
                        <div class="col-sm-5">
                    ...
                    <div id="allow_multiple_sizes-toggle">
                        
                        

                        <div class="form-group">
                            <label for="input-special-price" class="col-sm-3 control-label"></label>
                            <div class="col-sm-5">
                                    ...
                                    <div class="panel panel-default panel-table">
                                        <div class="table-responsive">
                                            <table class="table table-striped table-border table-sortable">
                                                ...
                                                <tbody>
                                                    <?php if(!empty($string_sizes)){ $table_row = sizeof($string_sizes);} else{ $table_row = 0;} ?>
                                                    <?php foreach ($string_sizes as $k =>$v ) { ?>
                                                        ...
                                                        <tr id="table-row<?php echo $table_row; ?>" >
                                                            ...
                                                            <td>
                                                                ...
                                                                <select name="sizes_v[<?php echo $table_row; ?>]" class="form-control">
                                                                ...
                                                                <?php echo form_error('option_size['.$table_row.'][value]', '<span class="text-danger">', '</span>'); ?>
                                                            </td>
                                                            <td>
                                                                <input type="text" name="option_size[<?php echo $table_row; ?>][price]" class="form-control" value="<?php echo $v; ?>"  />
                                                                <?php echo form_error('option_size['.$table_row.'][price]', '<span class="text-danger">', '</span>'); ?>
                                                            </td>
                            ...
                            <?php if ($menu_options) { ?>
                                <?php foreach ($menu_options as $menu_option) { ?>
                                    <div id="option<?php echo $option_row; ?>" class="tab-pane row wrap-all">
                                        ...
                                        <div class="panel panel-default panel-table hidden_div">
                                            <div class="table-responsive">
                                                <table class="table table-striped table-border table-sortable">
                                                    <thead>
                                                    <tr>
...
<div id="default_option_div" class="col-md-12">
    <div class="col-md-2" >
        <input type="checkbox" class="default_option_checkbox" name="menu_options[<?php echo $option_row; ?>][default_value_id]"
        <?php if (isset($option_values[$menu_option['option_id']])) { 
            $boolean_checked = false;
                foreach($menu_options[$option_row-1]['option_values'] as $def_value) {

                    // $menu_options as $menu_option
                    // $menu_option['option_values'] as $value
                    // if ($menu_option['default_value_id'] == $value['menu_option_value_id'])

                    if($menu_options[$option_row-1]['default_value_id'] && ($menu_options[$option_row-1]['default_value_id'] == $def_value['menu_option_value_id'])){
                ...
                // echo "<pre>";
                //     var_dump($menu_options[$option_row-1]['option_values']);
                    // var_dump($def_value);
                    // echo "</pre>";
                    // var_dump($def_value['menu_option_value_id']);
            }
        ?> 
        >
    ...
    <div class="col-md-3" >
        <select id="" class="form-control default_option_select" <?php if(!$boolean_checked){ echo 'disabled="true"'; } ?> name="menu_options[<?php echo $option_row; ?>][default_value_id]">
            <?php if (isset($option_values[$menu_option['option_id']])) { ?>
                <?php foreach($option_values[$menu_option['option_id']] as $option_value) { ?>
                    <option value="<?php $key = array_search($option_value['option_value_id'], array_column($menu_option['option_values'], 'option_value_id')); echo $menu_option['option_values'][$key]['menu_option_value_id']; ?>" <?php if($menu_options[$option_row-1]['default_value_id'] == $menu_option['option_values'][$key]['menu_option_value_id']){ echo 'selected'; } ?>><?php echo $option_value['value']; ?></option>
                <?php } ?>
            <?php } ?>
        </select>
    </div>
...
<script type="text/javascript"><!--
    $(document).ready(function() {
        ...
        $('input[name="allow_multiple_sizes"]').on('change', function() {
            if (this.value == '1') {
                $('#allow_multiple_sizes-toggle').slideDown(300);
                $('#input-price').closest(".form-group").hide();
                $('input[name="menu_price"]').val('0.0');
            } else {
                $('#allow_multiple_sizes-toggle').slideUp(300);
...
<script type="text/javascript"><!--
    ...
    function addOption(data) {
        ...
        html += '		</div>';
        html += '	</div>';
        html += '	<div class="panel panel-default panel-table hidden_div"><div class="table-responsive">';
        ...
        for (j = 0; j < data.option_values.length; j++) {
            modified_addOptionValue(option_row);
        }

        option_row++;
    }

    function modified_addOptionValue(option_row) {
        ...
        var tr_id = '#option-value' + option_value_row;
        $(tr_id).find('select').find('option:eq('+j+')').prop('selected', true).trigger('change');
...
function addValue() {
    ...
    html += '   <td class="action action-one"><a class="btn btn-danger" onclick="confirm(\'<?php echo lang('alert_warning_confirm'); ?>\') ? $(this).parent().parent().remove() : false;"><i class="fa fa-times-circle"></i></a></td>';
    html += '   <td>';
    html += '   <select name="sizes_v[' + table_row + ']" class="form-control">';
    html += $('#sizes_values').html();
    html += '   </select>';
    html += '   </td>';
    html += '   <td><input type="text" name="option_size[' + table_row + '][price]" class="form-control" /></td>';
    html += '</tr>';

    $('#size_values .table-sortable tbody').append(html);
    $('#table-row' + table_row + ' select.form-control').select2();
...
<script type="text/javascript">
    $('.default_option_checkbox').on('change', function(){
