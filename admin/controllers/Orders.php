...
class Orders extends Admin_Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$data['orders'][] = array(
				...
				'order_status_id'	=> $result['status_id'],
		...
		$data['statuses_custom'] = array();
		$statuses = $this->Statuses_model->getStatusesCustom('order');
		foreach ($statuses as $statuses) {
			$data['statuses_custom'][] = array(
				'status_id'			=> $statuses['status_id'],
				'status_name'		=> $statuses['status_name']
		...
		$data['modal'] = $this->Orders_model->get_modal_status();
	...
	public function check_order(){
		$orders = $this->Orders_model->getCustomCount($filter);
		$count = sizeof($orders);
		$data['orders'] = $orders;
		$data['count'] = $count;
		$data['order_list'] = array();
			$data_telephone = '';
			$data_customer_address = '';

		for($i = 0; $i < $count; $i++){

			$data['cart_items'] = array();
			$cart_items = $this->Orders_model->getOrderMenus($orders[$i]['order_id']);
	        $menu_options = $this->Orders_model->getOrderMenuOptions($orders[$i]['order_id']);
			foreach ($cart_items as $cart_item) {
				$option_data = array();
			...
			$data['order_list'][$i] = $data['cart_items'];

			$data['totals'] = array();
			$order_totals = $this->Orders_model->getOrderTotals($orders[$i]['order_id']);
			foreach ($order_totals as $total) {
				if ($order_info['order_type'] === '2' AND $total['code'] == 'delivery') {
					continue;
				}
			...
			$data['total'][$i] = $data['totals'];


			$id = $orders[$i]['order_id'];
			$order_info = $this->Orders_model->getOrder((int) $id);
			$data_telephone[$id] = $order_info['telephone'];
			...
			if (!empty($order_info['customer_id'])) {
				$customer_address = $this->Addresses_model->getAddress($order_info['customer_id'], $order_info['address_id']);
				$data_customer_address[$id] = $this->country->addressFormat($customer_address);
			} else if (!empty($order_info['address_id'])) {
				$customer_address = $this->Addresses_model->getGuestAddress($order_info['address_id']);
				$data_customer_address[$id] = $this->country->addressFormat($customer_address);
			}
		...
		$results = $this->Orders_model->getList();
		...
		foreach ($results as $result) {
			...
			$data['orders_list'][] = array(
				...
				'first_name'		=> $result['first_name'],
				'last_name'			=> $result['last_name'],
				'telephone'			=> $data_telephone[$result['order_id']],
				'address'			=> $data_customer_address[$result['order_id']],
				'order_status_id'	=> $result['status_id'],
				'order_status'		=> $result['status_name'],
				'status_color'		=> $result['status_color'],
				'order_total'		=> $this->currency->format($result['order_total']),
	...
	public function status_url(){
		...
		if($duration != null){
			if(is_int($duration) || $duration < 5 || $duration > 500){
				$duration = 10;
			}
			$data['duration'] = $duration;
		}
			
		$update = $this->Orders_model->update_modal_status($dataArray, $data);
		echo $update;
	}

	public function changeStatus(){
		$order_id = $this->input->post('order_id');
		$data['order_status'] = $this->input->post('status_id');

		$update = $this->Orders_model->updateOrder($order_id, $data);
		echo $update;
	}
